...
>
>2，添加数据时，计算key的**哈希码（hashCode）**确定元素在数组中的下标
>
>​	key相同则替换
>
>​	不同则存入链表或红黑树中
...
>
>
>1. **算 Hash**：用 `key.hashCode()` 的高 16 位异或低 16 位计算出最终 hash 值，目的是减少冲突。
>2. **找位置**：用 `(数组长度-1) & hash` 这个位运算，快速确定 key 在数组中的哪个桶。
>3. **插数据**：这是核心，分三种情况：
>   - **桶是空的**：直接新建一个节点放进去。
>   - **桶里有东西**：遍历这个桶里的链表（或树）。
>     - **找到相同的 key**：覆盖旧的 value。
>     - **没找到相同的 key**：把新节点加到链表末尾。**如果加完后链表长度超过 8，并且数组总长度达到 64，就把链表转成红黑树**（否则就优先扩容）。
>4. **看扩容**：插入新元素后，如果总元素数超过了`容量 * 0.75（负载因子）`，就**扩容两倍**，并重新排列所有元素。
